...
class ShowUML(models.TransientModel):
    ...
    installed = fields.Boolean(string="Installed", compute="_compute_installed")
    deps = fields.Binary(
        string="Dependencies", attachment=False
    )  # Do not use attachments for storage
    deps_fname = fields.Char(string="Dependencies")
    deps_preview = fields.Html(string="Preview")
    models = fields.Binary(string="Module Models", attachment=False)
    ...
    def show_pro_warning(self):
        if self.module_id:
            if not self.show_fields:
                ...
                raise MissingError(
                    _("Please purchase Pro version to use this feature!")
                )
